  ...
  for (date in dates) {
    ...
    for (i in 1:length(day_ind)) {
      ...
      if (length(dep_inds) == 0) {
        ...
        dep_delay <- sum(day_dat$DEP_DEL15[dep_inds]) / i # length(dep_inds) # leaving PIT
      }
      ...
      if (length(arr_inds) == 0) {
        ...
        arr_delay <- sum(day_dat$ARR_DEL15[arr_inds]) / i # length(arr_inds) # arriving to PIT 
      }
      ...
      for (c in 1:length(carriers)) {
        ...
        if (nrow(car_inds) == 0) {
          car_delay <- 0
            ...
            car_delay <- sum(car_inds$DEP_DEL15 | car_inds$ARR_DEL15) / i # nrow(car_inds)
          }
...
train_delay_ratio <- delay_ratio(train)
vis_delay_ratio <- delay_ratio(vis)
guess_delay_ratio <- delay_ratio(guess)
