...
models <- c("base rate", "mixture models", "linear model", "complete linkage clustering",
            "prototype clustering", "svm", "random forest")
...
model.table[2, 2] <- to.percent(.1202)
model.table[2, 3] <- to.percent(.0657)
model.table[2, 4] <- 0

# linear model
model.table[3, 2] <- to.percent(y.hat.2[1])
model.table[3, 3] <- to.percent(y.hat.2[2])
y.hat.lm <- predict(fit.lm.2, newdata = guesses.2016)
predictions.y.hat <- length(which(y.hat.lm > .5))
model.table[3, 4] <- predictions.y.hat
...
model.table[7, 2] <- to.percent(0.1699)
model.table[7, 3] <- to.percent(0.0916)
model.table[7, 4] <- 14